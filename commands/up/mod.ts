    ...
    poll(async () => {
      ...
      if (
        ...
        if (runs.ok && runs.data) {
          ...
          if (isDeploymentStable(runs.data)) {
            return {
              deployment: deployment.data,
...
function isDeploymentStable(
  runs: paths["/deployments/{id}/runs"]["get"]["responses"]["200"]["content"][
    "application/json"
  ],
): boolean {
  const filteredRuns = runs.filter((run) => run.replicas && run.replicas > 0);
  const healthyRun = filteredRuns.every((run) =>
    run.readyReplicas === run.replicas
  );
