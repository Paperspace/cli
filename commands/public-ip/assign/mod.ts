...
export const assign = command("assign", {
  short: "Assign a public IP",
  long: `
    Assign a public IP to a machine.
  `,
  commands: subCommands,
  args: args().tuple([
    z.string().describe("The public IP to assign"),
  ]).optional(),
  flags: flags({
    fields,
  }).merge(flags({
    "machine-id": flag({
      aliases: ["m"],
      short: "The ID of the machine to assign the public IP to",
    }).string().nullable(),
  })),
  ...
  async function* ({ args, flags }) {
    ...
    if (!ip) {
      ip = await input("IP:", {
        filter: (v) => !!v.sequence.match(/[0-9\.]/),
      });
      asserts(ip, "A public IP is required");
    }

    const response = await loading(
      publicIps.assign({ ip, machineId: flags["machine-id"] }),
    );
