...
export const restore = command("restore", {
  ...
  args: args().tuple([
    z.string().describe("The ID of the snapshot to restore to"),
  ]).optional(),
  flags: flags({
    fields,
  }).merge(flags({
    "create-snapshot": flag({
      short: "Create an additional snapshot before restore",
    }).boolean().optional(),
  })),
  ...
  async function* ({ args, flags }) {
    ...
    if (!id) {
      ...
      asserts(id, "A snapshot ID is required");
    }

    const response = await loading(
      snapshots.restore({
        id,
        createSnapshotBeforeRestore: flags["create-snapshot"],
      }),
    );
